...
class LookupServer : public Server,
 ...
 public:
  ...
  inline virtual void GetVerboseTransactionsForTxBlockI(
      const Json::Value& request, Json::Value& response) {
    response = this->GetVerboseTransactionsForTxBlock(request[0u].asString());
  }
  ...
  Json::Value GetVerboseTransactionsForTxBlock(const std::string& txBlockNum);
};
